<template>
    <div class="contentChatPage d-flex p-relative">
        <span class="clearChat p-absolute font-bold pointer" v-if="clear" @click="clearChat">@Clear_Chat_{{ nameChat }}</span>
            ...
            <div class="mess p-10 space-between p-relative">
                ...
                <div class="messages" ref="containerMessage">
                    <p v-for="(chat, index) in chats"
                      :class="{myMessage: chat.id_user === admin_id, someMessage: chat.id_user !== admin_id}" class="p-relative">
                        <q
                            class="p-absolute pointer"
                            :class="{meQ: chat.id_user === admin_id,notMeQ: chat.id_user !== admin_id}"
                            @click="visitUser(chat.id_user)"
                            v-if="chat.id_user !== admin_id"
                        >
                            {{ chat.username }}
                        </q>
    ...
    methods: {
        SendMessage () {
            if (this.chatMood) {
                ...
                this.messageSending = '';
                let data = {
                    user_id: window.admin.id,
                    message: message,
                    course_id: this.Course_id,
                    username: 'Admin',
                };
                let addMessageNow = {
                    id_user : this.admin_id,
                    message : message,
                };
        ...
        clearChat() {
            axios.post(`/Admin/Clear/Chat?id=${this.Course_id}`)
            .then (response => {
                if (JSON.stringify(this.chats) !== response.data.message) {
                    this.chats = JSON.parse(response.data.message);
                    this.scrollToBottom();
